...
async def ban_user(chat_id, i, rights):
    try:
        await jmisbest(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@jmisbest.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@jmisbest.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@jmisbest.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
...
async def _(event):
    ...
    if user.id == 6556390631:
        await event.reply(
            "حاضر سيدي وتاج راسي جناب السيد الموقر محمد ريفز حفظه الله ☻️"
        )
...
@jmisbest.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await jmisbest(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in jmisbest.iter_participants(chat_id):
        ...
        await jmisbest.send_message(chat_id, usrtxt)
...
@jmisbest.ar_cmd(pattern="الغاء التفليش", groups_only=True)
