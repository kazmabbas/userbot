...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗟𝗘𝗮𝗗𝗲𝗥 - اوامر الزخرفه ](t.me/V_P_N_8) 𓆪\n\n"
    " ⏎ اهلا بك في اوامر الزخرفه\n"
    " ⏎لاستعمال الامر ⏎ .زخرفه + اسم الي تريدون\n"
    " تريدون زخرفته  ملحوظه عليك الاشتراك بقنوات التي ستظهر امامك \n"
    "\n𓆩 [•𝑬3𝒅𝑨𝒎• 𝙇𝙚𝘼𝙙𝙀𝙧 ™⤶](t.me/POSTS_E3DAM) 𓆪"
    "\n𓆩 [𝙈𝙊𝙉𝙕𝙀𝙍™𝗹𝗲𝗮𝗱𝗲𝗿 •](t.me/V_P_N_8) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت القائد انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝗟𝗘𝗮𝗗𝗲𝗥 :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت القائد انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "كامل" or input_str[0] == "الكل":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your LEader__.\n"
                )
            else:
