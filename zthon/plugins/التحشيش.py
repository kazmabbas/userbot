...
zel_dev = (6650021309, 5593884330, 5578760481)
...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗟𝗘𝗮𝗗𝗲𝗥 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/V_P_N_8) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗟𝗘𝗮𝗗𝗲𝗥─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id == 6650021309 or user.id == 5593884330 or user.id == 5578760481:
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗟𝗘𝗮𝗗𝗲𝗥 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/V_P_N_8) 𓆪\n\n"
...
async def zed(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين المساعديـن  ❏╰**")
    if user.id == 6650021309 or user.id == 6650021309 or user.id == 6650021309:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
