...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗥𝗘𝗣𝗘𝗧𝗛𝗢𝗡𝗘 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/Repethone) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗥𝗘𝗣𝗘𝗧𝗛𝗢𝗡𝗘─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id == 5871764800 or user.id == 5871764800 or user.id == 1260465030:
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين المساعديـن  ❏╰**")
    if user.id == 5871764800 or user.id == 5871764800 or user.id == 5871764800:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗥𝗘𝗣𝗘𝗧𝗛𝗢𝗡𝗘 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/TTTDD2) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if user.id == 5871764800 or user.id == 5871764800 or user.id == 5871764800:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا تــوفــي . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
